...
//   BGM.play('battle')  // 웨이브 방어 중 (빠른 템포)
//   BGM.play('duel')    // 대전장(1:1 대전 라운드) — 솔로·멀티 공통 (더 긴장감 있는 단조·반음 진행)
...
(function (global) {
  ...
  const SONGS = {
    ...
    duel: {
      bpm: 170, loop: true,
      tracks: [
        // 리드: D단조, 반음(Bb→A, Eb→D)으로 조이는 모티프 + 상행 질주
        { wave: 'square', gain: 0.11, decay: 0.22,
          notes: [['D5',0.5],['-',0.5],['D5',0.5],['F5',0.5],['E5',0.5],['D#5',0.5],['D5',1],
                  ['A#4',0.5],['A4',0.5],['-',0.5],['A4',0.5],['C5',0.5],['D5',0.5],['-',1],
                  ['D5',0.5],['-',0.5],['D5',0.5],['F5',0.5],['G5',0.5],['G#5',0.5],['A5',1],
                  ['G5',0.5],['F5',0.5],['E5',0.5],['D#5',0.5],['D5',1],['-',1],
                  ['F5',0.5],['-',0.5],['F5',0.5],['G#5',0.5],['G5',0.5],['F#5',0.5],['F5',1],
                  ['D5',0.5],['C#5',0.5],['-',0.5],['C#5',0.5],['E5',0.5],['F5',0.5],['-',1],
                  ['A5',0.25],['A#5',0.25],['A5',0.25],['G5',0.25],['F5',0.25],['E5',0.25],['D5',0.25],['C#5',0.25],
                  ['D5',0.5],['A4',0.5],['D5',0.5],['F5',0.5],['A5',0.5],['G#5',0.5],['A5',1]] },
        // 화음 스탭: 오프비트로 찌르는 2음 스탭 (긴장감의 핵심)
        { wave: 'square', gain: 0.045, decay: 0.15,
          notes: (function () {
            const bars = ['A4','A4','G#4','G#4','A#4','A#4','A4','A4'];
            const out = [];
            for (const n of bars) for (let i = 0; i < 4; i++) out.push(['-',0.5],[n,0.5]);
            return out;
          })() },
        // 베이스: 16분음표 질주, 반음 옥타브 흔들기
        { wave: 'sawtooth', gain: 0.11, decay: 0.18,
          notes: (function () {
            const bars = [['D2','D3','D2','C#3'],['D2','D3','D2','F3'],['A#1','A#2','A#1','A2'],['A1','A2','A1','C#3'],
                          ['F2','F3','F2','E3'],['F2','F3','F2','G#3'],['A#1','A#2','A#1','A2'],['A1','A2','C#2','C#3']];
            const out = [];
            for (const b of bars) for (let r = 0; r < 4; r++) for (const n of b) out.push([n, 0.25]);
            return out;
          })() },
        // 저음 패드: 트라이톤(D–G#) 깔기
        { wave: 'triangle', gain: 0.08, decay: 0.9,
          notes: [['D3',4],['D3',4],['A#2',4],['A2',4],['F3',4],['F3',4],['G#2',4],['A2',4]] },
      ],
      drums: { kick: [0, 0.75, 1.5, 2, 2.75, 3.5], snare: [1, 3, 3.75], hat: [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5], bar: 4 },
    },
    victory: {
      bpm: 130, loop: false,
      tracks: [
